...
def parse_args():
    ...
    parser.add_argument('-Lorb',metavar='Lorb',dest='Lorb',type=int,default=3,help='set Lorb')
...
#                    f.write('%3d %23.16e %23.16e\n' % \
#                        (((j*L+i)*Lorb+(Lorb-1-orb1))*Lorb+(Lorb-1-orb2),fij[j,i,orb1,orb2].real,fij[j,i,orb1,orb2].imag))
    f.close()
...
##                    print('%3d %3d %3d %3d %3d %23.16e %23.16e 0.0 ' % \
##                        (j,i,orb1,orb2,((j*L+i)*Lorb+(Lorb-1-orb1))*Lorb+(Lorb-1-orb2),fij[j,i,orb1,orb2].real,fij[j,i,orb1,orb2].imag))
    f.close()
...
def func_enek(kx,ky):
    e1 = -(1.0+np.exp(1j*kx))
    e2 = -(1.0+np.exp(1j*ky))
    e3 = -(1.0+np.exp(1j*(ky-kx)))
    ham = np.array([[0,e1,e2],[np.conjugate(e1),0,e3],[np.conjugate(e2),np.conjugate(e3),0]])
    ene, vec = np.linalg.eigh(ham)
#    print("kx,ky,ene",kx,ky,ene)
#    print("vec",vec[:,0],vec[:,1],vec[:,2])
#    return ene, vec
